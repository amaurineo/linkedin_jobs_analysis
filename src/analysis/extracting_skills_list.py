...
class SkillExtractor:
    ...
    def normalize_text(self, text: str) -> str:
        ...
        try:
            ...
            logger.warning(f"Could not normalize text: '{text[:50]}...'. Error: {e}")
    ...
    def process_dataframe(self, df: pd.DataFrame, text_column: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        '''
        ...
        try:
            ...
            df_processed['skills_found'] = df_processed[text_column].progress_apply(self.extract_skills)
            ...
            for index, row in df_processed.iterrows():
                ...
                try:
                    for skill in extracted_skills_list:
                        if skill:
                            skills_data.append({'job_id': job_id_val, 'skill': skill})
                except Exception as e:
                    logger.warning(f'Error processing skills for job_id {job_id_val}: {str(e)}. Skills list: {extracted_skills_list}')
            ...
            logger.info(f'Skill extraction completed. Found {len(df_skills)} skill entries.')
